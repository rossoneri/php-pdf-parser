...
class Reader
{
    ...
    public function get_page_count($pages=null) {
        if (!$pages) {
    ...
    public function get_object($indirect_ref) {
        if(isset($this->resolved_objects[$indirect_ref->generation])){
            $retval = $this->resolved_objects[$indirect_ref->generation];
            if ($retval && isset($retval[$indirect_ref->idnum])) {
    ...
    public function read_object_header($stream) {
    ...
    public function cache_indirect_object($generation, $idnum, $obj) {
            ...
            if ($x == "x") {
                ...
                while (1) {
                    ...
                    while ($cnt < $size) {
                        ...
                        if (!isset($this->xref[$generation][$num])) {
    ...
    public function get_xmp_metadata() {
    ...
    public function read_next_end_line($stream) {
        $line = '';
        while (true) {
            $x = fread($stream, 1);
